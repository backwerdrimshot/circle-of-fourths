...
import { getKeyboardWindow } from "./keyboard-window.mjs";
...
function keyboard(key, box) {
  ...
  const keyboardWindow = getKeyboardWindow(midis);
  const sx = (box.width - 26) / keyboardWindow.width;
  const sy = 0.17;
  const startX = box.x + 13;
  ...
  let markup = `<g role="img" aria-label="${escape(key.label)} major scale on a xylophone; about one and a half octaves shown; one tonic-to-tonic octave highlighted; outlined bars mark the starting and ending tonic; notes ${escape(notes.join(", "))}${alternate ? `; enharmonic ${escape(alternate.label)} major: ${escape(scaleOctave(alternate).notes.join(", "))}` : ""}">`;
  ...
  for (const bar of keyboardWindow.bars) {
...
export function createPosterSvg(size = "letter") {
  ...
  svg += '<desc id="poster-description">C starts at twelve o’clock. Move clockwise through the flat keys. Fifteen written major keys occupy twelve positions, including the enharmonic pairs D-flat/C-sharp, G-flat/F-sharp, and C-flat/B. Each spelling has its own key signature and relative minor. Each xylophone shows about one and a half octaves of graduated percussion bars, with its range chosen around the scale; exactly eight green bars trace one ascending tonic-to-tonic octave, with note names printed on those bars. A thick outline marks the starting and ending tonic. C major and A minor share a signature with no sharps or flats, with C and A as their respective home notes. Enharmonic spellings use the same physical bars: C-flat is B, F-flat is E, E-sharp is F, and B-sharp is C.</desc>';
  ...
  svg += text(width - 60, legendY + 27, "About 1½ octaves shown · one octave highlighted", 13, 'text-anchor="end"');
